...
class website_sale(http.Controller):
    ...
    def payment_get_status(self, sale_order_id, **post):
        ...
        if not order:
            ...
            return request.website._render("website_sale.order_state_message", values)
        ...
        return request.website._render("website_sale.order_state_message", values), values
    ...
    @http.route(['/shop/print_saleorder'], type='http', auth="public", website=True)
    def print_saleorder(self):
        cr, uid, context = request.cr, SUPERUSER_ID, request.context
        sale_order_id = request.session.get('sale_last_order_id')
        if sale_order_id:
            order = request.registry['sale.order'].browse(cr, uid, sale_order_id, context=context)
            pdf = request.registry['report'].get_pdf(cr, uid, [order.id], 'sale.report_saleorder', data=None, context=context)
