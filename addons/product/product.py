...
class product_product(osv.osv):
    ...
    def _set_suppliers(self, cr, uid, id, name, value, args, context=None):
        if not value:
            return
        product_tmpl_id = self.browse(cr, uid, id, context=context)[0].product_tmpl_id.id
        SUPPLIER = self.pool['product.supplierinfo']
        for supplier in value:
            if supplier[0] == 0:
                supplier[-1].update({'product_tmpl_id': product_tmpl_id, 'product_id': id})
                SUPPLIER.create(cr, uid, supplier[-1], context=context)
    ...
    def _get_suppliers(self, cr, uid, ids, name, args, context=None):
        ...
        for id in ids:
            product_tmpl_id = self.browse(cr, uid, id, context=context)[0].product_tmpl_id.id
            supplier_ids = SUPPLIER.search(cr, uid, ['|', ('product_id', '=', id), '&', ('product_id', '=', False), ('product_tmpl_id', '=', product_tmpl_id)], context=context)
            res[id] = supplier_ids
        return res
