...
class survey_mail_compose_message(osv.TransientModel):
    ...
    def onchange_multi_email(self, cr, uid, ids, multi_email, context=None):
        ...
        if error_message:
            raise UserError(_('Warning!'), _("One email at least is incorrect: %s" % error_message))
    ...
    def send_mail(self, cr, uid, ids, context=None):
        ...
        for wizard in self.browse(cr, uid, ids, context=context):
            ...
            if wizard.body.find("__URL__") < 0:
                raise UserError(_('Warning!'), _("The content of the text don't contain '__URL__'. \
            ...
            if not len(emails_list) and not len(partner_list):
                if wizard.model == 'res.partner' and wizard.res_id:
                    return False
                raise UserError(_('Warning!'), _("Please enter at least one valid recipient."))
