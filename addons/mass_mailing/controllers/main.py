...
class MassMailController(http.Controller):
    ...
    def full_url_redirect(self, code, stat_id, **post):
        cr, uid, context = request.cr, request.uid, request.context
        request.registry['website.alias.click'].add_click(cr, uid, code, request.httprequest.remote_addr, request.session['geoip'].get('country_code'), stat_id=stat_id, context=context)
        return werkzeug.utils.redirect(request.registry['website.alias'].get_url_from_code(cr, uid, code, context=context), 301)
