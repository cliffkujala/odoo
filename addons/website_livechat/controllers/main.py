...
class website_rating_livechat(http.Controller):
    ...
    def channel_rating(self, channel, **kw):
        # expose the ratings of the channel
        if channel.website_published:

            # get the last 100 ratings and the repartition per grade
            ratings = request.env['rating.rating'].search([('res_model','=','im_chat.session'), ('res_id', 'in', channel.session_ids.ids)], order='create_date desc', limit=100)
            repartition = channel.session_ids.rating_get_grades()
