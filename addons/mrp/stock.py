...
class StockMove(osv.osv):
    ...
    def check_tracking(self, cr, uid, move, lot_id, context=None):
        ...
        if move.product_id.track_production and (move.location_id.usage == 'production' or move.location_dest_id.usage == 'production') and not lot_id:
            raise Warning(_('Warning!'), _('You must assign a serial number for the product %s') % (move.product_id.name))
        if move.raw_material_production_id and move.location_dest_id.usage == 'production' and move.raw_material_production_id.product_id.track_production and not move.consumed_for:
            raise Warning(_('Warning!'), _("Because the product %s requires it, you must assign a serial number to your raw material %s to proceed further in your production. Please use the 'Produce' button to do so.") % (move.raw_material_production_id.product_id.name, move.product_id.name))
    ...
    def action_consume(self, cr, uid, ids, product_qty, location_id=False, restrict_lot_id=False, restrict_partner_id=False,
        ...
        if product_qty <= 0:
            raise Warning(_('Warning!'), _('Please provide proper quantity.'))
        ...
        for move in self.browse(cr, uid, ids2, context=context):
            ...
            if move_qty <= 0:
                raise Warning(_('Error!'), _('Cannot consume a move with negative or zero quantity.'))
...
class stock_warehouse(osv.osv):
    ...
    def _get_manufacture_pull_rule(self, cr, uid, warehouse, context=None):
        ...
        if not manufacture_route_id:
            raise Warning(_('Error!'), _('Can\'t find any generic Manufacture route.'))
