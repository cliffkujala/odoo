...
class stock_return_picking(osv.osv_memory):
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        if pick:
            if pick.state != 'done':
                raise UserError(_('Warning!'), _("You may only return pickings that are Done!"))
            ...
            if len(result1) == 0:
                raise UserError(_('Warning!'), _("No products to return (only lines in Done state and not fully returned yet can be returned)!"))
    ...
    def _create_returns(self, cr, uid, ids, context=None):
        ...
        for data_get in data_obj.browse(cr, uid, data['product_return_moves'], context=context):
            ...
            if not move:
                raise UserError(_('Warning !'), _("You have manually created product lines, please delete them to proceed"))
        ...
        if not returned_lines:
            raise UserError(_('Warning!'), _("Please specify at least one non-zero quantity."))
