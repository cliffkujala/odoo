...
class account_move_line(osv.osv):
    ...
    def _query_get(self, cr, uid, obj='l', context=None):
        ...
        if initial_bal and not context.get('periods', False) and not where_move_lines_by_date:
            ...
            raise Warning(_('Warning!'),_("You have not supplied enough arguments to compute the initial balance, please select a period and a journal in the context."))
        ...
        for obj_line in self.browse(cr, uid, ids, context=context):
            ...
            if obj_line.analytic_account_id:
                if not obj_line.journal_id.analytic_journal_id:
                    raise Warning(_('No Analytic Journal!'),_("You have to define an analytic journal on the '%s' journal!") % (obj_line.journal_id.name, ))
    ...
    def _check_no_closed(self, cr, uid, ids, context=None):
        ...
        for l in lines:
            if l.account_id.type == 'closed':
                raise Warning(_('Error!'), _('You cannot create journal items on a closed account %s %s.') % (l.account_id.code, l.account_id.name))
        return True
    ...
    def reconcile_partial(self, cr, uid, ids, type='auto', context=None, writeoff_acc_id=False, writeoff_period_id=False, writeoff_journal_id=False):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            if company_list and not line.company_id.id in company_list:
                raise Warning(_('Warning!'), _('To reconcile the entries company should be the same for all entries.'))
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            if line.reconcile_id:
                raise Warning(_('Warning'), _("Journal Item '%s' (id: %s), Move '%s' is already reconciled!") % (line.name, line.id, line.move_id.name))
            if line.reconcile_partial_id:
                for line2 in line.reconcile_partial_id.line_partial_ids:
                    if line2.state != 'valid':
                        raise Warning(_('Warning'), _("Journal Item '%s' (id: %s) cannot be used in a reconciliation as it is not balanced!") % (line2.name, line2.id))
    ...
    def reconcile(self, cr, uid, ids, type='auto', writeoff_acc_id=False, writeoff_period_id=False, writeoff_journal_id=False, context=None):
        ...
        if len(r) != 1:
            raise Warning(_('Error'), _('Entries are not of the same account or already reconciled ! '))
        if not unrec_lines:
            raise Warning(_('Error!'), _('Entry is already reconciled.'))
        account = account_obj.browse(cr, uid, account_id, context=context)
        if not account.reconcile:
            raise Warning(_('Error'), _('The account is not defined to be reconciled !'))
        if r[0][1] != None:
            raise Warning(_('Error!'), _('Some entries are already reconciled.'))

        if (not currency_obj.is_zero(cr, uid, account.company_id.currency_id, writeoff)) or \
           (account.currency_id and (not currency_obj.is_zero(cr, uid, account.currency_id, currency))):
            if not writeoff_acc_id:
                raise Warning(_('Warning!'), _('You have to provide an account for the write off/exchange difference entry.'))
            if writeoff > 0:
                debit = writeoff
                credit = 0.0
    ...
    def write(self, cr, uid, ids, vals, context=None, check=True, update_check=True):
        ...
        if vals.get('account_tax_id', False):
            raise Warning(_('Unable to change tax!'), _('You cannot change the tax, you should remove and recreate lines.'))
        if ('account_id' in vals) and not account_obj.read(cr, uid, vals['account_id'], ['active'])['active']:
            raise Warning(_('Bad Account!'), _('You cannot use an inactive account.'))
        if update_check:
    ...
    def _update_journal_check(self, cr, uid, journal_id, period_id, context=None):
        ...
        for (state,) in result:
            if state == 'done':
                raise Warning(_('Error!'), _('You can not add/modify entries in a closed period %s of journal %s.' % (period.name,journal.name)))
        if not result:
    ...
    def _update_check(self, cr, uid, ids, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            if line.move_id.state <> 'draft' and (not line.journal_id.entry_posted):
                raise Warning(_('Error!'), _('You cannot do this modification on a confirmed entry. You can just change some non legal fields or you must unconfirm the journal entry first.\n%s.') % err_msg)
            if line.reconcile_id:
                raise Warning(_('Error!'), _('You cannot do this modification on a reconciled entry. You can just change some non legal fields or you must unreconcile first.\n%s.') % err_msg)
    ...
    def create(self, cr, uid, vals, context=None, check=True):
        ...
        if 'period_id' in context and not isinstance(context.get('period_id', ''), (int, long)):
            ...
            if len(period_candidate_ids) != 1:
                raise Warning(_('Error!'), _('No period found or more than one period found for the given date.'))
        ...
        if not move_id:
            ...
            if not vals.get('move_id', False):
                if journal.sequence_id:
                    ...
                    raise Warning(_('No Piece Number!'), _('Cannot create an automatic sequence for this piece.\nPut a sequence in the journal definition for automatic numbering or create a sequence manually for this piece.'))
        ...
        if not ok:
            raise Warning(_('Bad Account!'), _('You cannot use this general account in this journal, check the tab \'Entry Controls\' on the related journal.'))
