...
openerp.website.if_dom_contains('div.o_website_alias_chart', function() {

    var QWeb = openerp.qweb;
    var website = openerp.website;

    openerp.website_url_chart = {};
    
    openerp.website_url_chart.BarChart = openerp.Widget.extend({
    ...
    openerp.website_url_chart.PieChart = openerp.Widget.extend({
   ...
   website.ready().done(function() {
        ...
        var clicks = website.session.model('website.alias.click');
        var alias_domain = ['alias_id', '=', parseInt(alias_id)];

        var total_clicks = function() {
            return clicks.call('search_count', [[alias_domain]]);
        };

        var clicks_by_day = function() {
            return clicks.call('read_group', [[alias_domain], ['create_date']],
                               {'groupby':'create_date:day'});
        };

        var clicks_by_country = function() {
            return clicks.call('read_group',  [[alias_domain], ['country_id']], 
                               {'groupby':'country_id'});
        };

        var last_week_clicks_by_country = function() {
            var interval = moment().subtract(7, 'days').format("YYYY-MM-DD");
            return clicks.call('read_group', [[alias_domain, ['create_date', '>', interval]], ['country_id']],
                               {'groupby':'country_id'});
        };
        ...
        .done(function(total_clicks, clicks_by_day, clicks_by_country, last_week_clicks_by_country, last_month_clicks_by_country) {
            ...
            if(total_clicks) {
                ...
                var all_time_chart = new openerp.website_url_chart.BarChart('#all_time_clicks_chart', begin_date, now, formatted_clicks_by_day);
                all_time_chart.start();

                // Process month line chart data
                var begin_date = moment().subtract(30, 'days');
                var month_chart = new openerp.website_url_chart.BarChart('#last_month_clicks_chart', begin_date, now, formatted_clicks_by_day);
                month_chart.start();

                // Process week line chart data
                var begin_date = moment().subtract(7, 'days');
                var week_chart = new openerp.website_url_chart.BarChart('#last_week_clicks_chart', begin_date, now, formatted_clicks_by_day);
                week_chart.start();

                // Process pie charts
                var all_time_pie_chart = new openerp.website_url_chart.PieChart('#all_time_countries_charts', clicks_by_country);
                all_time_pie_chart.start();

                var last_month_pie_chart = new openerp.website_url_chart.PieChart('#last_month_countries_charts', last_month_clicks_by_country);
                last_month_pie_chart.start();

                var last_week_pie_chart = new openerp.website_url_chart.PieChart('#last_week_countries_charts', last_week_clicks_by_country);
        ...
        $('.copy-to-clipboard').on('click', function(e) {
            ...
            if(!animating_copy) {
                ...
                $('.o_website_alias_short_url').clone()
                    ...
                    .appendTo($('.o_website_alias_short_url'))
                    .animate({
                        opacity: 0,
                        bottom: "+=20",
                    }, 500, function() {
