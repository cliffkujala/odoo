...
openerp.website.if_dom_contains('div.o_website_alias_create_tracked_url', function() {
    ...
    openerp.website_url.RecentLinkBox = openerp.Widget.extend({
        template: 'website_alias.RecentLink',
        events: {
            'click .btn_shorten_url_clipboard':'toggle_copy_button',
            'click .o_website_alias_edit_code':'edit_code',
            'click .o_website_alias_ok_edit':function(e) {
                e.preventDefault();
                this.submit_code();
            },
            'click .o_website_alias_cancel_edit':function(e) {
                e.preventDefault();
                this.cancel_edit();
            },
            'submit #o_website_alias_edit_code_form':function(e) {
                e.preventDefault();
                this.submit_code();
            },
        },
        ...
        toggle_copy_button: function() {
            ...
            if(!this.animating_copy) {
                ...
                var top = this.$('.o_website_alias_short_url').position().top;
                this.$('.o_website_alias_short_url').clone()
                    .css('position', 'absolute')
                    .css('left', 15)
                    .css('top', top-2)
                    .css('z-index', 2)
                    .removeClass('o_website_alias_short_url')
                    .addClass('animated-link')
                    .insertAfter(this.$('.o_website_alias_short_url'))
                    .animate({
                        opacity: 0,
                        top: "-=20",
                    }, 500, function() {
        ...
        edit_code: function() {
            var init_code = this.$('#o_website_alias_code').html();

            this.$('#o_website_alias_code').html("<form style='display:inline;' id='o_website_alias_edit_code_form'><input type='hidden' id='init_code' value='" + init_code + "'/><input type='text' id='new_code' value='" + init_code + "'/></form>");
            this.$('.o_website_alias_edit_code').hide();
            this.$('.copy-to-clipboard').hide();
            this.$('.o_website_alias_edit_tools').show();
        },
        cancel_edit: function() {
            this.$('.o_website_alias_edit_code').show();
            this.$('.copy-to-clipboard').show();
            this.$('.o_website_alias_edit_tools').hide();
            self.$('.o_website_alias_code_error').hide();

            var old_code = this.$('#o_website_alias_edit_code_form #init_code').val();
            this.$('#o_website_alias_code').html(old_code);

            this.$('#code-error').remove();
            this.$('#o_website_alias_code form').remove();
        },
        submit_code: function() {
            var self = this;

            var init_code = this.$('#o_website_alias_edit_code_form #init_code').val();
            var new_code = this.$('#o_website_alias_edit_code_form #new_code').val();

            function show_new_code(new_code) {
                self.$('.o_website_alias_code_error').html('');
                self.$('.o_website_alias_code_error').hide();

                self.$('#o_website_alias_code form').remove();

                // Show new code
                var host = self.$('#o_website_alias_host').html();
                self.$('#o_website_alias_code').html(new_code);

                // Update button copy to clipboard
                self.$('.btn_shorten_url_clipboard').attr('data-clipboard-text', host + new_code)
                
                // Show action again
                self.$('.o_website_alias_edit_code').show();
                self.$('.copy-to-clipboard').show();
                self.$('.o_website_alias_edit_tools').hide();
            }
            ...
            if(init_code == new_code) {
                ...
                openerp.jsonRpc('/website_alias/add_code', 'call', {'init_code':init_code, 'new_code':new_code})
                    .then(function(result) {
                        show_new_code(result[0].code);
                    })
                    .fail(function(error) {
                        self.$('.o_website_alias_code_error').show();
                        self.$('.o_website_alias_code_error').html("This code is already taken");
                    }) ;
            }
        },
    });
    ...
    openerp.website_url.RecentLinks = openerp.Widget.extend({
        ...
        get_recent_links: function(filter) {
            ...
            openerp.jsonRpc('/website_alias/recent_links', 'call', {'filter':filter, 'limit':20})
                .then(function(result) {
        ...
        update_notification: function() {
            if(this.getChildren().length == 0) {
                ...
                $('.o_website_alias_recent_links_notification').html("<div class='alert alert-info'>" + message + "</div>");
            }
            else {
                $('.o_website_alias_recent_links_notification').empty();
            }
        },
    });
    ...
    openerp.website.ready().done(function() {
        ...
        ZeroClipboard.config({swfPath: location.origin + "/website_alias/static/src/js/ZeroClipboard.swf" });

        // UTMS selects widgets
        var campaign_select = new openerp.website_url.SelectBox('crm.tracking.campaign');
        campaign_select.start($("#campaign-select"), 'e.g. Promotion of June, Winter Newsletter, ..');

        var medium_select = new openerp.website_url.SelectBox('crm.tracking.medium');
        medium_select.start($("#channel-select"), 'e.g. Newsletter, Social Network, ..');

        var source_select = new openerp.website_url.SelectBox('crm.tracking.source');
        source_select.start($("#source-select"), 'e.g. Search Engine, Website page, ..');

        // Recent Links Widgets
        var recent_links;
        openerp.website.add_template_file('/website_alias/static/src/xml/recent_link.xml')
            .then(function() {
                recent_links = new openerp.website_url.RecentLinks();
                recent_links.appendTo($("#o_website_alias_recent_links"));
        ...
        $("#o_website_alias_link_tracker_form").submit(function(event) {
            ...
            openerp.jsonRpc("/website_alias/new", 'call', params)
                .then(function (result) {
                    if('error' in result) {
                        // Handle errors
