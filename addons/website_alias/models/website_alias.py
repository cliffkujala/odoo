...
class website_alias(models.Model):
    """website_alias allow users to wrap any URL into a short and trackable URL
    via a frontend and backend interface. website_alias counts clicks on each tracked link.
    ...
    @api.one
    @api.depends('alias_click_ids.alias_id')
    ...
    def action_view_statistics(self):
        action = self.env['ir.actions.act_window'].for_xml_id('website_alias', 'action_view_click_statistics')
    ...
    def create(self, vals):
        ...
        alias = super(website_alias, self).create(vals)

        code = self.env['website.alias.code'].get_random_code_string()
        self.env['website.alias.code'].create({'code': code, 'alias_id': alias.id})
    ...
    def get_url_from_code(self, code, context=None):
        ...
        return code_rec.alias_id.redirected_url
...
class website_alias_code(models.Model):
    ...
    alias_id = fields.Many2one('website.alias', 'Alias', required=True, ondelete='cascade')
...
class website_alias_click(models.Model):
    ...
    def add_click(self, code, ip, country_code, stat_id=False):
        self = self.sudo()
        code_rec = self.env['website.alias.code'].search([('code', '=', code)])

        if not code_rec:
            return None

        again = self.search_count([('alias_id', '=', code_rec.alias_id.id), ('ip', '=', ip)])

        if not again:
            country_record = self.env['res.country'].search([('code', '=', country_code)], limit=1)

            vals = {
                'alias_id': code_rec.alias_id.id,
